...
| Agent | 职责 | 交付物 |
|-------|------|--------|
| **Developer** | 端到端开发 | 代码 + 自测报告 + 测试命令 |
| **QA** | 验收测试+诊断 | 测试报告（通过/问题诊断+修复方案） |
| **Release** | Git推送 | 推送报告 |
| **Main** | 协调+兜底 | 向用户汇报结果 |
...
1. **用户提需求** → Main接收
2. **Main拆解** → 启动Developer
3. **Developer开发** → 代码+自测报告
4. **Main检查** → 启动QA
5. **QA测试** → 
   - 通过 → 通知Release推送
   - 失败 → 诊断问题，通知Developer修复 → 回到步骤3
6. **Release推送** → GitHub
7. **Main汇报** → 用户确认
